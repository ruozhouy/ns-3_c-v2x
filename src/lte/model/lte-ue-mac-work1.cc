  ...
  if (allocIt != m_discTxPools.m_psdchTx.end() && (*allocIt).subframe.frameNo == frameNo && (*allocIt).subframe.subframeNo == subframeNo)
  {
	...
	for(poolIt2 = m_sidelinkTxPoolsMapV2x.begin(); poolIt2 != m_sidelinkTxPoolsMapV2x.end(); poolIt2++)
	{
		...
		{				
			...
			if(poolIt2->second.m_pool->GetSchedulingType() == SidelinkCommResourcePoolV2x::UE_SELECTED)
			{
				...
				if(randVal < m_probResourceKeep && firstTx == false)
				{
					...
					for (uint32_t idxCam = 0; idxCam < m_numCam; idxCam++) {   // m_reselSubCtr = m_numCam
						...
						while (!frameNoOverflow && (txInfosAll[idxCam].subframe.frameNo < frameNo || (txInfosAll[idxCam].subframe.frameNo == frameNo && txInfosAll[idxCam].subframe.subframeNo < subframeNo))) {
							txInfosAll[idxCam].subframe.frameNo += (txInfosAll[idxCam].subframe.subframeNo + m_pRsvp) / 10;
							if (txInfosAll[idxCam].subframe.frameNo > 1024) {
								frameNoOverflow = true;
								txInfosAll[idxCam].subframe.frameNo %= 1024;
							}
							txInfosAll[idxCam].subframe.subframeNo = (txInfosAll[idxCam].subframe.subframeNo + m_pRsvp) % 10;
						}
					}
					// !ryu5
					//-------------------------------
				...
				else
				{
					...
					while (setPickedTxOpportunities.size() < m_numCam) { // Get a set of random tx resources to use; assuming resources are non-overlapping (should be based on debug output of txOpps)
						...
						if (setPickedSubframes.find(subframe_hash) != setPickedSubframes.end()) continue;
					...
					for (uint32_t idxCam = 0; idxCam < m_numCam; idxCam++) {   // for each picked resource, assign it to the corresponding txInfosAll
						txOppsIt = txOpps.begin(); 
					...
					std::sort(txInfosAll.begin(), txInfosAll.end(), 
							[](const SidelinkCommResourcePoolV2x::SidelinkTransmissionInfo & a, const SidelinkCommResourcePoolV2x::SidelinkTransmissionInfo & b){
				...
				poolIt2->second.m_grant_received = true;
				if (poolIt2->second.m_nextGrantsAll.empty()) poolIt2->second.m_nextGrantsAll.resize(m_numCam);
				for (uint32_t idxCam = 0; idxCam < m_numCam; idxCam++) {   // for each picked resource, assign it to the corresponding txInfosAll
			...
			if(poolIt2->second.m_grant_received)
			{
				...
				// std::cout << "-=-=-=-=-=- m_subchLen: " << (int)m_subchLen 
				// 			<< " | m_sizeSubchannel: " << (int)m_sizeSubchannel 
				// 			<< " | m_startRbSubchannel: " << (int)m_startRbSubchannel 
				// 			<< " | m_numSubchannel: " << (int)m_numSubchannel 
				// 			<< " | m_pRsvp: " << (int)m_pRsvp 
				// 			<< " | m_rnti: " << (int)m_rnti 
				// 			<< std::endl;
				...
				if (poolIt2->second.m_pscchTxsAll.empty()) poolIt2->second.m_pscchTxsAll.resize(m_numCam);
				if (poolIt2->second.m_psschTxsAll.empty()) poolIt2->second.m_psschTxsAll.resize(m_numCam);
				for (uint32_t idxCam = 0; idxCam < m_numCam; idxCam++) {
				...
				if (m_numCam > 1) {
					...
					int hasMoreTxToAdd = m_numCam; // number of numCam that has more tx in the queue
					std::vector< std::list<SidelinkCommResourcePoolV2x::SidelinkTransmissionInfo>::iterator > pscchIts (m_numCam);
					std::vector< std::list<SidelinkCommResourcePoolV2x::SidelinkTransmissionInfo>::iterator > psschIts (m_numCam);
					for (uint32_t idxCam = 0; idxCam < m_numCam; idxCam++) {
				...
				allocItPssch != poolIt2->second.m_psschTx.end() && (*allocItPssch).subframe.frameNo == frameNo && (*allocItPssch).subframe.subframeNo == subframeNo)
		{
			...
			//std::cout << " New PSCCH transmission, m_reselCtr = " << int(m_reselCtr) << ", m_reselSubCtr = " << int(m_reselSubCtr) << std::endl; // ryu5: m_reselSubCtr
