...
namespace ns3 {
  ...
  void 
  SidelinkCommResourcePoolV2x::SetPool (LteRrcSap::SlCommResourcePoolV2x pool)
  {
    // not implemented yet 
  } 

  void 
  SidelinkCommResourcePoolV2x::SetPool (LteRrcSap::SlV2xPreconfigCommPool pool)
  {
    //preconfigured pools are always UE selected
  ...
  {
    uint16_t sizeSubchhanel = LteRrcSap::sizeSubchannelAsInt(m_sizeSubchannel);  
    uint16_t numSubchannel = LteRrcSap::numSubchannelAsInt(m_numSubchannel);
    bool adjacency = LteRrcSap::adjacencyAsBool(m_adjacencyPscchPssch);
    uint16_t startRbSubchannel = LteRrcSap::startRbSubchannelAsInt(m_startRbSubchannel);
    uint16_t prb; 
    ...
    else 
    {
      ...
      uint16_t pscchStart = LteRrcSap::startRbPscchPoolAsInt(m_startRbPscchPool); // only available when adjacency is not enabled
      for(uint16_t m=0; m<numSubchannel; m++){
        for(uint16_t j=0; j<=1; j++){
  ...
  {
    uint16_t sizeSubchannel = LteRrcSap::sizeSubchannelAsInt(m_sizeSubchannel);
    uint16_t numSubchannel = LteRrcSap::numSubchannelAsInt(m_numSubchannel);
    uint16_t startRbSubchannel = LteRrcSap::startRbSubchannelAsInt(m_startRbSubchannel);
    uint16_t prb; 
  ...
  { 
    ...
    uint16_t sizeSubch = LteRrcSap::sizeSubchannelAsInt(m_sizeSubchannel);
    uint16_t startRbSubch = LteRrcSap::startRbSubchannelAsInt(m_startRbSubchannel); 
    uint16_t startRbPscch = LteRrcSap::startRbPscchPoolAsInt(m_startRbPscchPool); 
  ...
  SidelinkCommResourcePoolV2x::GetPsschTransmissions (SidelinkCommResourcePoolV2x::SubframeInfo subframe, uint8_t riv, uint16_t pRsvp, uint8_t sfGap, uint8_t reTxIdx, uint8_t pscchResource, uint8_t reselCtr)
  {
    ...
    uint8_t subchReTxIdx = tmp[1]; // index for the subchannel of the retransmission
    uint16_t sizeSubch = LteRrcSap::sizeSubchannelAsInt(m_sizeSubchannel);
    uint16_t startRbSubch = LteRrcSap::startRbSubchannelAsInt(m_startRbSubchannel); // start of the resource pool for transmission
...
TypeId 
  ...
  void
  SidelinkTxCommResourcePoolV2x::SetPool (LteRrcSap::SlCommResourcePoolV2x pool)
  {
  ...
  void
  SidelinkTxCommResourcePoolV2x::SetPool (LteRrcSap::SlV2xPreconfigCommPool pool)
  {
    SidelinkCommResourcePoolV2x::SetPool (pool);
    m_index = 0;
    m_mcs = 0;
